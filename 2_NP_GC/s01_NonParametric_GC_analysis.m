...
for gg = 1:size(useCoordPairs,1)
    ...
    savename = ['allEB_DetrendFirst)_DiffVoxels_' num2str(theseTrials) '_CohPower_GC_' num2str(possCoords{1}) ...
        '_and_' num2str(possCoords{2}) '_TopVoxel_FINAL.mat']; %savename
    ...
    for hh = 1:length(possCoords)
        ...
        listCoords = ['CoordSortedEBRem_NewROIS_FixedTstat_' num2str(possCoords{hh}) '_by_TstatOverTime_0_to_500ms'];
        ...
        for voxel = 1:size(coordList,1)
            ...
            filename = ['Source_Waveform_at_' num2str(thisVoxel) '_for_ALL_trials_fmSae_faces_aEBfspmeeg_7MJN_0to500ms_trial_205.mat'];
        ...
        for voxel = 1:size(coordList,1)
            thisVoxel = coordList(voxel,:);
            filename = ['Source_Waveform_at_' num2str(thisVoxel) '_for_ALL_trials_fmSae_random_aEBfspmeeg_7MJN_0to500ms_trial_354.mat'];
            load ([filepath filename])
