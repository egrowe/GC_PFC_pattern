...
if ~exist('costRange','var')
    costRange = [-20:5:15]; % default cost range if not specified
end
...
for reps = 1:nReps
    ...
    randSplitF = randperm(size(data_faces,2)); %select trials to use randomly
    train70F = randSplitF(1:trainSplit); % select 70% train trials: Faces
    test30F = randSplitF(trainSplit+1:end); % select 30% test trials: Faces
    randSplitR = randperm(size(data_random,2)); %select trials to use randomly
    train70R = randSplitR(1:trainSplit); % select 70% train trials: Random
    test30R = randSplitR(trainSplit+1:end); % select 30% test trials: Random
